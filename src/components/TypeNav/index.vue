...
<script>
// 引入lodash全部功能函数中的 throttle(节流)方法
...
export default {
  ...
  methods: {
    ...
    changeIndex: throttle(function (index) {
      ...
      // 正常情况下（用户慢慢的操作）: 鼠标进入，每一个一级分类, 都会触发鼠标进入事件
      // 非正常事件 （用户操作过快）: 本身全部的一级分类都应该触发鼠标进入事件, 但是经过测试, 只有部分触发了
    ...
    goSearch(event) {
      ...
      // 获取到当前触发这个事件的节点【h3、a、dt、dl】, 需要带有data-categoryname 这样节点【一定是a标签】
