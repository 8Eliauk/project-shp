...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      // 置空ID
      // 带给服务器参数说明是可有可无的：如果属性值为空的字符串还是把相应的字段带给服务器
      // 但是你把相应的字段变为undefined,当前这个字段不会带给服务器
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      this.getData();
      // 地址栏也需要更改, 进行路由的跳转(现在的路由跳转跳转到自己这里)
      // 严谨：本意是删除query,如果路径当中出现params不应该删除,路由跳转的时候应该戴着
    ...
    removeKeyword() {
      ...
      this.$bus.$emit("clear")
      // 地址栏也需要更改, 进行路由的跳转(现在的路由跳转跳转到自己这里)
      // 严谨：本意是删除params,如果路径当中出现query不应该删除,路由跳转的时候应该戴着
      if(this.$route.query) {
    ...
    // 排序的操作
    // 原理： 
        // 第一次执行 点击(综合按钮) 如果flag(1)和 初始值的originFlag(1) 相等,那么就执行第一条判断,改变排序方式,
        // 当点击(价格按钮),这时 flag(2) != 初始值originFlag(1) 执行第二条判断 更新初始值的originFlag为(2)默认排序方式为降序
        // 这时第二次点击(价格)的时候 flag(2) ==  originFlag(2) 所以走第一条判断 从而改变升序和降序
    changeOrder(flag) {
      // flag形参：它是一个标记,代表用户点击的是综合(1) 还是价格(2) 【用户点击的时候传递进来】
      // 这里获取的是最开始的状态
  ...
  watch: {
    ...
    $route() {
      ...
      // 在此发起ajax请求
      this.getData();
      // 每一次请求完毕,应该把相应的1、2、3级分类的id置空,让它接收下一次的相应1、2、3
